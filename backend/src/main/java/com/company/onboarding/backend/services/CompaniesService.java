...
public Integer getCompanyIdByName(String companyName) {
    ...
    try (Connection connection = DBConfig.getConnection();
        ...
        e.printStackTrace();
        System.err.println("Error getting company ID by name: " + e.getMessage());
        return null;
    }
}

public Timestamp getCompanyCreatedAt(int companyId) {
    String sql = "SELECT created_at FROM companies WHERE company_id = ?";
    try (Connection connection = DBConfig.getConnection();
         PreparedStatement stmt = connection.prepareStatement(sql)) {
        ...
        e.printStackTrace();
        System.err.println("Error getting created_at: " + e.getMessage());
        return null;
    }
}
